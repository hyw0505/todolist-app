...
| **기본 언어** | 한국어 (ko) |
| **언어 감지** | 미설정 시 브라우저 `Accept-Language` 헤더 기반 자동 감지 |
| **언어 설정 위치** | 클라이언트 `localStorage` (`i18nextLng` 키) |
| **적용 범위** | UI 텍스트 (프론트엔드 전용, API 응답 메시지 제외) |
...
| **i18n** | react-i18next | 최신 | 다국어 지원 (ko/en/jp) |
...
| v0.4 | 2026-04-02 | Yongwoo | **다국어 지원 추가**: §1.3 범위, §4.5 국제화 요구사항(신설), §5.2 i18n 기술 스택, §8.1 UI 포함 기능 반영 |
