...
LanguageSelector와 동일한 스타일을 사용한다. 아이콘(🌙/☀️) + 텍스트 레이블(dark/light)을 함께 표시한다.
...
| 크기 | `16px` | GNB 내 사용 |
| 색상 | `#FFFFFF` | GNB 배경 위 흰색 |
| 레이블 | `light` / `dark` | 아이콘 우측에 텍스트 표시 |
| 호버 | `rgba(255,255,255,0.1)` | LanguageSelector와 동일 |
...
ThemeToggle은 LanguageSelector와 동일한 버튼 스타일을 사용한다. 아이콘 + 텍스트 레이블 형태로 표시한다.
...
| v1.3 | 2026-04-03 | Yongwoo | **테마 토글 스타일 변경**: §5.4·§6·§12.1.1 — 원형→직사각형(4px), 아이콘+텍스트(dark/light), LanguageSelector와 동일한 GNB 버튼 스타일 적용 |
